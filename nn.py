...
class NN:
    ...
    def compute_gradients(self, X, Y):
        ...
        for idx in range(num_layers-1):
            ...
            # dim * batch_size (dot) weight  + bias 

            Z = np.matmul(ws[idx], D) + bs[idx]  # (hidden_dim, batch_size)
            D = self.activation_function.activate(Z)  # keep (hidden_dim, batch_size)
            D_stack.append(D)

# Experiment with matmul () 
        Yhat = np.matmul(ws[-1], D) + bs[-1]
        ...
        for idx in range(num_layers-2, -1, -1):
            ...
            grad = np.matmul(ws[idx + 1].T, grad)
            grad *= self.activation_function.backprop_grad(D_stack[idx + 1])
